...
function makeTestDoc(): MockDocumentData {
	return {
		paragraphs: [
			{ text: "Introduction", style: "Heading 1", outlineLevel: "OutlineLevel1" },
			{ text: "This document outlines our strategy for Q4.", style: "Normal", outlineLevel: "OutlineLevelBodyText" },
			{ text: "Market Analysis", style: "Heading 2", outlineLevel: "OutlineLevel2" },
			{ text: "The market grew by 15% year over year.", style: "Normal", outlineLevel: "OutlineLevelBodyText" },
			{ text: "Revenue grew 20% in the APAC region.", style: "Normal", outlineLevel: "OutlineLevelBodyText" },
			{ text: "Conclusion", style: "Heading 1", outlineLevel: "OutlineLevel1" },
			{ text: "We recommend proceeding with the expansion plan.", style: "Normal", outlineLevel: "OutlineLevelBodyText" },
		],
...
describe("word_get_outline", () => {
	it("returns headings up to maxDepth", async () => {
		const result = await processCommand("c1", "word_get_outline", { maxDepth: 2 }) as any;
...
describe("word_get_paragraphs", () => {
	it("returns paragraphs from startIndex with count", async () => {
		const result = await processCommand("c4", "word_get_paragraphs", { startIndex: 0, count: 3 }) as any;
	...
	it("returns all paragraphs by default", async () => {
		const result = await processCommand("c5", "word_get_paragraphs", {}) as any;
...
describe("word_get_selection", () => {
	it("returns selected text with paragraph context", async () => {
		const result = await processCommand("c6", "word_get_selection", {}) as any;
...
describe("word_search", () => {
	it("finds matching text", async () => {
		const result = await processCommand("c7", "word_search", { searchText: "Revenue" }) as any;
	...
	it("returns 0 for no matches", async () => {
		const result = await processCommand("c8", "word_search", { searchText: "xyzzy" }) as any;
...
describe("word_replace_text", () => {
	it("replaces text in a paragraph", async () => {
		const result = await processCommand("c9", "word_replace_text", {
			paragraphIndex: 3, oldText: "15%", newText: "20%",
		}) as any;
	...
	it("returns error if oldText not found", async () => {
		const result = await processCommand("c10", "word_replace_text", {
			paragraphIndex: 3, oldText: "nonexistent", newText: "x",
		}) as any;
	...
	it("returns error for out-of-range index", async () => {
		const result = await processCommand("c11", "word_replace_text", {
			paragraphIndex: 99, oldText: "x", newText: "y",
		}) as any;
...
describe("word_insert_text", () => {
	it("inserts at end of document", async () => {
		const result = await processCommand("c12", "word_insert_text", {
			text: "New final paragraph", insertLocation: "end",
		}) as any;
	...
	it("inserts after a specific paragraph", async () => {
		const result = await processCommand("c13", "word_insert_text", {
			text: "Inserted after intro", insertLocation: "afterParagraph", paragraphIndex: 0,
		}) as any;
...
describe("word_add_comment", () => {
	...
	it("adds comment to a specific paragraph", async () => {
		const result = await processCommand("c15", "word_add_comment", {
			commentText: "Reword this", paragraphIndex: 4,
		}) as any;
