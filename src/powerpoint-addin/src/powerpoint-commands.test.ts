...
const mockBridge = {
	reportResult: async (_cmd: string, _success: boolean, _err?: string, _payload?: unknown) => {},
};

vi.mock("./communication", () => ({
	reportResult: (cmd: string, success: boolean, err?: string, payload?: unknown) =>
		...
		slides: [
			{
				id: "slide_0",
				shapes: [
					{ id: "s1", name: "Title 1", type: "TextBox", text: "Quarterly Results", left: 50, top: 30, width: 600, height: 60, font: { name: "Calibri", size: 36, bold: true } },
					{ id: "s2", name: "Content 1", type: "TextBox", text: "Revenue grew 15%", left: 50, top: 120, width: 600, height: 200 },
					{ id: "s3", name: "Picture 1", type: "Image", left: 50, top: 350, width: 300, height: 200 },
					{ id: "s4", name: "Slide Number Placeholder", type: "TextBox", text: "1", left: 700, top: 500, width: 30, height: 20 },
				],
			...
			},
			{
				id: "slide_1",
				shapes: [
					{ id: "s5", name: "Title 1", type: "TextBox", text: "Pricing Table", left: 50, top: 30, width: 600, height: 60 },
					{
						id: "s6", name: "Table 1", type: "Table", left: 50, top: 120, width: 600, height: 200,
						tableCells: [["Product", "Price", "Stock"], ["Widget A", "$10", "100"], ["Widget B", "$20", "50"]],
					},
				],
				notes: "",
			},
...
describe("powerpoint_get_deck_outline", () => {
	it("returns all slides with shape properties", async () => {
		const result = await processCommand("cmd-1", "powerpoint_get_deck_outline", {}) as any;
	...
	it("skips non-content shapes for title detection", async () => {
		const result = await processCommand("cmd-2", "powerpoint_get_deck_outline", {}) as any;
		// "Slide Number Placeholder" should be skipped for title
		expect(result.slides[0].title).toBe("Quarterly Results");
	});

	it("falls back to 'Slide N' for slides without titles", async () => {
		const result = await processCommand("cmd-3", "powerpoint_get_deck_outline", {}) as any;
...
describe("powerpoint_get_slide", () => {
	it("returns full shape properties for a slide", async () => {
		const result = await processCommand("cmd-4", "powerpoint_get_slide", { slideIndex: 0 }) as any;
	...
	it("includes font properties for text shapes", async () => {
		const result = await processCommand("cmd-5", "powerpoint_get_slide", { slideIndex: 0 }) as any;
		const title = result.shapes[0];
	...
	it("returns error for out-of-range index", async () => {
		const result = await processCommand("cmd-6", "powerpoint_get_slide", { slideIndex: 99 }) as any;
...
describe("powerpoint_get_slide_image", () => {
	it("returns base64 PNG image", async () => {
		const result = await processCommand("cmd-7", "powerpoint_get_slide_image", { slideIndex: 0 }) as any;
...
describe("powerpoint_get_shape_image", () => {
	it("returns image for a shape by ID", async () => {
		const result = await processCommand("cmd-8", "powerpoint_get_shape_image", { slideIndex: 0, shapeId: "s3" }) as any;
	...
	it("returns error for missing shape", async () => {
		const result = await processCommand("cmd-9", "powerpoint_get_shape_image", { slideIndex: 0, shapeId: "nonexistent" }) as any;
...
describe("powerpoint_get_table", () => {
	it("returns table cells as 2D array", async () => {
		const result = await processCommand("cmd-10", "powerpoint_get_table", { slideIndex: 1, shapeId: "s6" }) as any;
	...
	it("returns error for missing shape", async () => {
		const result = await processCommand("cmd-11", "powerpoint_get_table", { slideIndex: 1, shapeId: "nope" }) as any;
		expect(result.error).toContain("not found");
	});
});
...
describe("powerpoint_get_selection", () => {
	it("returns none when nothing selected", async () => {
		const result = await processCommand("cmd-12", "powerpoint_get_selection", {}) as any;
...
describe("powerpoint_get_speaker_notes", () => {
	it("returns notes for a specific slide", async () => {
		const result = await processCommand("cmd-13", "powerpoint_get_speaker_notes", { slideIndex: 0 }) as any;
		expect(result.notes).toHaveLength(1);
		expect(result.notes[0].notes).toBe("Talk about revenue growth");
	});

	it("returns empty string for slides without notes", async () => {
		const result = await processCommand("cmd-14", "powerpoint_get_speaker_notes", { slideIndex: 1 }) as any;
...
describe("powerpoint_update_shape_text", () => {
	it("updates text on a shape", async () => {
		const result = await processCommand("cmd-15", "powerpoint_update_shape_text", {
			slideIndex: 0, shapeId: "s1", text: "Q4 Results",
		}) as any;
	...
	it("returns error for missing shape", async () => {
		const result = await processCommand("cmd-16", "powerpoint_update_shape_text", {
			slideIndex: 0, shapeId: "nope", text: "test",
		}) as any;
	...
	it("returns error for non-text shape", async () => {
		const result = await processCommand("cmd-17", "powerpoint_update_shape_text", {
			slideIndex: 0, shapeId: "s3", text: "test", // s3 is an Image
		}) as any;
...
describe("powerpoint_update_shape_properties", () => {
	it("updates position properties", async () => {
		const result = await processCommand("cmd-18", "powerpoint_update_shape_properties", {
			slideIndex: 0, shapeId: "s1", left: 100, top: 50,
		}) as any;
	...
	it("updates font properties", async () => {
		const result = await processCommand("cmd-19", "powerpoint_update_shape_properties", {
			slideIndex: 0, shapeId: "s1", fontSize: 48, bold: true,
		}) as any;
...
describe("powerpoint_update_speaker_notes", () => {
	it("writes notes to a slide", async () => {
		const result = await processCommand("cmd-20", "powerpoint_update_speaker_notes", {
			slideIndex: 1, notes: "New speaker notes",
		}) as any;
...
describe("powerpoint_add_textbox", () => {
	it("creates a new text box", async () => {
		const result = await processCommand("cmd-21", "powerpoint_add_textbox", {
			slideIndex: 0, text: "Hello!", left: 100, top: 200, width: 300, height: 50,
		}) as any;
...
describe("powerpoint_add_image", () => {
	it("creates a new image shape", async () => {
		const result = await processCommand("cmd-22", "powerpoint_add_image", {
			slideIndex: 0, imageBase64: "data:image/png;base64,abc123", left: 100, top: 200,
		}) as any;
...
describe("powerpoint_add_table", () => {
	it("creates a new table shape", async () => {
		const result = await processCommand("cmd-23", "powerpoint_add_table", {
			slideIndex: 0, rows: 3, columns: 4, left: 100, top: 200,
		}) as any;
...
describe("powerpoint_add_slide", () => {
	it("adds a new slide at the end", async () => {
		const result = await processCommand("cmd-25", "powerpoint_add_slide", {}) as any;
	...
	it("adds a new slide at specific index", async () => {
		const result = await processCommand("cmd-26", "powerpoint_add_slide", { atIndex: 1 }) as any;
...
describe("powerpoint_delete_slide", () => {
	it("deletes a slide", async () => {
		const result = await processCommand("cmd-27", "powerpoint_delete_slide", { slideIndex: 1 }) as any;
...
describe("powerpoint_move_slide", () => {
	it("moves a slide to new position", async () => {
		const result = await processCommand("cmd-28", "powerpoint_move_slide", { fromIndex: 0, toIndex: 2 }) as any;
...
describe("unknown command", () => {
	it("returns error for unknown command", async () => {
		const result = await processCommand("cmd-99", "nonexistent_command", {}) as any;
